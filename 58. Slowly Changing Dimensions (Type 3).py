...
dbutils.fs.rm("dbfs:/FileStore/tables/Data/DeltaType2", True)
...
# MAGIC  Prev_FirsName string,
# MAGIC  Prev_LastName string,
# MAGIC  Prev_Address string
# MAGIC )
# MAGIC using DELTA
# MAGIC LOCATION 'dbfs:/FileStore/tables/Data/DeltaType2'
...
# MAGIC when not matched then insert (tar.Customer_id, tar.firstname, tar.lastname,  tar.address, tar.Prev_FirsName,  tar.Prev_LastName , tar.Prev_Address  ) values (src.customer_id, src.firstname, src.lastname,  src.address, NULL,  NULL, NULL)
...
# MAGIC --on tar.Customer_id = src.customer_id
# MAGIC --when matched and (src.firstname <> tar.firstname or src.lastname <> tar.lastname or src.address <> tar.address) then 
# MAGIC --update set tar.Customer_id = src.Customer_id , tar.Prev_FirsName =  tar.firstname,  tar.firstname = src.firstname , tar.PREV_LASTNAME =  tar.lastname,  tar.lastname = src.lastname, tar.PREV_ADDRESS =  tar.Address, tar.address = src.address
# MAGIC --when not matched then insert  (tar.Customer_id, tar.firstname, tar.lastname,  tar.address, tar.Prev_FirsName,  tar.Prev_LastName , tar.Prev_Address  ) values (src.Customer_id, src.firstname, src.lastname,  src.address, NULL, NULL, NULL)
# MAGIC
...
# MAGIC using v_customers as src
# MAGIC on tar.Customer_id = src.customer_id
# MAGIC when matched and (src.firstname <> tar.firstname) then
# MAGIC update set  tar.Prev_FirsName =  tar.firstname,  tar.firstname = src.firstname 
# MAGIC when matched and (src.lastname <> tar.lastname) then
# MAGIC update set  tar.PREV_LASTNAME =  tar.lastname,  tar.lastname = src.lastname 
# MAGIC when matched and (src.Address <> tar.Address) then
# MAGIC update set  tar.PREV_ADDRESS =  tar.Address,  tar.Address = src.Address 
# MAGIC when not matched then insert  (tar.Customer_id, tar.firstname, tar.lastname,  tar.address, tar.Prev_FirsName,  tar.Prev_LastName , tar.Prev_Address  ) values (src.Customer_id, src.firstname, src.lastname,  src.address, NULL, NULL, NULL)
